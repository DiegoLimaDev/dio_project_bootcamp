...
import 'package:my_app/repositories/profile_data_repository.dart';
...
class ProfilePageHive extends StatefulWidget {
  const ProfilePageHive({super.key});

  @override
  State<ProfilePageHive> createState() => _ProfilePageHiveState();
}

class _ProfilePageHiveState extends State<ProfilePageHive> {
  late ProfileDataRepo profileDataRepo;
  ProfileDataModel profileDataModel = ProfileDataModel.empty();
  ...
  void loadData() async {
    profileDataRepo = await ProfileDataRepo.load();
    profileDataModel = profileDataRepo.getData();
    nameController.text = profileDataModel.name ?? '';
    birthDayController.text = profileDataModel.birthday.toString();
    selectedLevel = profileDataModel.levelExp ?? '';
    selectedLangs = profileDataModel.langs;
    salary = profileDataModel.salary ?? 0;
    experienceTime = profileDataModel.expTime ?? 0;
